...
/**
 * The BankAccount class models a bank account. It represents a bank account by
 * its owner, type of account, balance, interest rate, and the date that account
 * is opened
 */
public class BankLoan implements Comparable {
    ...
    /**
     * Constructor that allow BankLoan objects to be created by providing values for
     * owner, loan amount, interest rate, and open date and paid off date.
     * loanNumber increments each time this constructor is called
     * accrueInterestCounter is initially set to 0, and lastAccrueInterestRateDate
     * is set to null
     * 
     ...
     * @throws InvalidInterestRateForLoan throw when initialInterestRate is not
     *                                    within 5% and 20%
     * @throws LimitOnLoansExceeded       throw when number of loans exceeds number
     *                                    of accounts
     */
    ...
    /**
     * Constructor that allow BankLoan objects to be created by providing values for
     * owner, loan amount, interest rate, and open date. Paid off date is initially
     * set to null. startingLoanNumber increments each time this constructor is
     * called lastAccrueInterestRateDate is set to null loanNumber is set based on
     * the number of BankLoan objects created
     * 
     * @param initialOwner        owner of loan
     * @param initialLoanAmount   loan amount of this BankLoan object
     * @param initialInterestRate interest rate of the loan
     ...
     */
    public BankLoan(BankCustomer initialOwner, double initialLoanAmount, double initialInterestRate,
            LocalDate initialOpenedDate) throws InvalidInterestRateForLoan, LimitOnLoansExceeded {
     ...
     * owner, loan amount, interest rate, and open date. Paid off date is initially
     * set to null. startingLoanNumber increments each time this constructor is
     * called loanNumber is set based on the number of BankLoan objects created
     * 
     * @param initialOwner              owner of loan
     * @param initialLoanAmount         loan amount of this BankLoan object
     * @param initialInterestRate       interest rate of the loan
     ...
     */
    public BankLoan(BankCustomer initialOwner, double initialLoanAmount, double initialInterestRate,
            LocalDate initialOpenedDate, LocalDate initialAccrueInterestDate)
            throws InvalidInterestRateForLoan, LimitOnLoansExceeded {
        owner = initialOwner;
        owner.add(this);
        loanAmount = initialLoanAmount;

        // if initialInterestRate is not within 5% and 20%, throw
        // InvalidInterestRateForLoan exception
    ...
    public BankLoan(int initialLoanNumber, double initialAmount, double initialInterestRate,
            LocalDate initialOpenedDate, LocalDate initialPaidOffDate, BankCustomer initialOwner, int initialCustomerNo)
            throws InvalidInterestRateForLoan, LimitOnLoansExceeded {
    ...
    /**
     * mutator that accept an amount via its parameter, then decrease the balance of
     * the loan
     * 
    ...
    public void makePayment(double paymentAmount) throws IllegalArgumentException {
        ...
        // if paymentAmount is negative, and loan has not been paid off throw
    ...
    public void accrueInterest() throws UnableToAccrueNow {
        ...
        // If this is the first time the account is accrued, and loan has not been paid
        // off proceed
        ...
        // Proceed if it is and loan has not been paid off, if not, throw
        // UnableToAccrueNow exception
    ...
    public String toString() {
        return String.format("Loan #%d: %s owned by %s, interest rate %.1f%%.", loanNumber,
                formatter.format(loanAmount), getOwner().getName(), interestRate);
    }
